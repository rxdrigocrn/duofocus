...
const TaskTable = ({ title, tasks, bgColor = 'bg-white', textColor = 'text-gray-800', onFilter, activeFilter }) => {
    return (
            ...
            <div className="flex justify-between items-center mb-5">
                ...
                <div className="flex gap-2 text-gray-500">
                    <button className="cursor-pointer" onClick={() => onFilter('dumbbell')}>
                        <Dumbbell
                            size={28}
                            className={`${activeFilter === 'dumbbell' ? 'text-blue-600' : 'text-gray-500'}`}
                        />
                    </button>
                    ...
                    <button className="cursor-pointer" onClick={() => onFilter('brain')}>
                        <Brain
                            size={28}
                            className={`${activeFilter === 'brain' ? 'text-blue-600' : 'text-gray-500'}`}
                        />
                    </button>
                </div>
            </div>
